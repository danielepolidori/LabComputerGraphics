...
float intorno = 0.01f;   // Facilita il riconoscere che ci si trova sopra un punto
float *puntatore_x = nullptr;   // Puntatore al punto da spostare
float *puntatore_y = nullptr;
...
void myMouseFunc(int button, int state, int x, int y) {
	...
	if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN)
	{
		...
		if (state == GLUT_DOWN) {
			...
			// Controllo se viene premuto un punto

			for (int i = 0; i < NumPts; i++) {			// Scorro la curva di base

				// Se mi trovo sopra (l'intorno di) un punto
				if (xPos < PointArray[i][0] + intorno &&
				    xPos > PointArray[i][0] - intorno &&
				    yPos < PointArray[i][1] + intorno &&
				    yPos > PointArray[i][1] - intorno) {

					// Il puntatore punta alle coordinate di quel punto
					puntatore_x = &PointArray[i][0];
					puntatore_y = &PointArray[i][1];
				}
			}

			if (puntatore_x == nullptr && puntatore_y == nullptr) {   // Se non e' stato associato un punto della curva di base

				for (int j = 0; j < NumPts3; j++) {		// Scorro il tratto cubico

					// Se mi trovo sopra (l'intorno di) un punto
					if (xPos < PointArray3[j][0] + intorno &&
					    xPos > PointArray3[j][0] - intorno &&
					    yPos < PointArray3[j][1] + intorno &&
					    yPos > PointArray3[j][1] - intorno) {

						// Il puntatore punta alle coordinate di quel punto
						puntatore_x = &PointArray3[j][0];
						puntatore_y = &PointArray3[j][1];
					}
				}
			}
		}
...
// Aggiorna le coordinate del punto che si sta spostando
void myMotionFunc(int x, int y) {
	...
	if (puntatore_x != nullptr && puntatore_y != nullptr) {   // Conferma che ci si trova sopra un punto
...
void addNewPoint(float x, float y) {
	...
	if (NumPts3 == MaxNumPts3) {   // Se il tratto cubico e' completo
		...
		for (int i = 1; i < NumPts3; i++) {   // Non considero i=0 perche' v0~PointArray3[0] scompare (sostituito da p3)
...
int main(int argc, char** argv)
{
	...
	glutMotionFunc(myMotionFunc);   // Per catturare il trascinamento con il mouse
