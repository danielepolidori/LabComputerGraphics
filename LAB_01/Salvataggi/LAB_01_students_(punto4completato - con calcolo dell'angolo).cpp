...
void addNewPoint(float x, float y) {
	...
	if (NumPts3 == MaxNumPts3) {   // Se il tratto cubico e' completo
		...
		if (NumPts == 0) {   // Se e' il primo tratto cubico (i.e. ancora non c'e' una curva di base) considero anche i=0
		...
		// Gestisco i=1 in base alla continuita' selezionata
		/*if (continuita == 0) {						// Continuita' C0

			PointArray[NumPts][0] = PointArray3[1][0];   // x
			PointArray[NumPts][1] = PointArray3[1][1];   // y
			PointArray[NumPts][2] = 0;

			NumPts++;
		}*/
		else if (continuita == 1) {					// Continuita' C1

			/* ... --> p2 --> p3=v0 --> v1 --> ...
			 * Devo calcolare l'angolo tra p2-p3-v1:
			 *  - Traslo i punti cosi' che p3=v0 coincida con l'origine (0,0).
			 *  - Controllo se l'angolo e' di 180 gradi.
			 */ 
			...
			// ultimoCurvaBase  =  p2 - p3  =  vec2(p2_x - p3_x, p2_y - p3_y)
			//  * Corrisponde alle coordinate di p2, facendo coincidere p3=v0 con l'origine
			vec2 ultimoCurvaBase(PointArray[NumPts-2][0] - PointArray[NumPts-1][0], PointArray[NumPts-2][1] - PointArray[NumPts-1][1]);

			// primoTrattoCubico  =  v1 - v0  =  v1 - p3  =  vec2(v1_x - p3_x, v1_y - p3_y)
			//  * Corrisponde alle coordinate di v1, facendo coincidere p3=v0 con l'origine
			vec2 primoTrattoCubico(PointArray3[1][0] - PointArray[NumPts-1][0], PointArray3[1][1] - PointArray[NumPts-1][1]);


			//float dotProduct = dot(ultimoCurvaBase, primoTrattoCubico);
			//float angleRad = acos(dotProduct / ultimoCurvaBase.length() / primoTrattoCubico.length());
			float dotProduct = dot(normalize(ultimoCurvaBase), normalize(primoTrattoCubico));
			float angleRad = acos(dotProduct);
			float angleDeg = angleRad / PI * 180.0f;
			printf("\nangleDeg: %f\n\n", angleDeg);

			if (angleDeg != 180.0f) {   // Se ho solo la continuita' C0
// ** le linee qui sopra non servono **
			...
			cout << "\nERRORE: valore della variabile `continuita` non compatibile.\n\n";
		}

		for (int i = 1; i < NumPts3; i++) {   // Non considero i=0 perche' il primo punto scompare (sostituito dall'ultimo della curva base)
		//for (int i = 2; i < NumPts3; i++) { 
